...
rb_dict = {df_2019['PLAYER'].iloc[i]: [df_2019['FPTS/G'].iloc[i]] for i in range(len(df_2019))
...
for i in range(len(df_2020)):
    if df_2020['PLAYER'].iloc[i] in rb_dict:
        rb_dict[df_2020['PLAYER'].iloc[i]].append(df_2020['FPTS/G'].iloc[i])
        
# create new dictionary that only holds data for players with more than one season
new_rb_dict = {x:y for x,y in rb_dict.items() if len(y) > 1}
...
players = [x.rstrip() for x in new_rb_dict]
players = pd.Series(players)

fpts_2019 = [x[0] for x in new_rb_dict.values()]
fpts_2019 = pd.Series(fpts_2019)

fpts_2020 = [x[1] for x in new_rb_dict.values()]
